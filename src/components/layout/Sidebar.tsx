...
const Sidebar: React.FC = () => {
  ...
  const handleRenamePlaylist = (id: string) => {
    const playlistsNew = playlists.map(p => p.id === id ? { ...p, name: renameValue } : p);
    localStorage.setItem('user_playlists', JSON.stringify(playlistsNew));
    setPlaylists(playlistsNew);
  ...
  return (
    <aside className="w-64 bg-black flex flex-col overflow-hidden">
      ...
      <div className="p-6 border-t border-gray-800 space-y-4">
        ...
        <NavLink to="/liked-songs" className={({ isActive }) => `sidebar-item cursor-pointer flex items-center gap-2 ${isActive ? 'text-spotify-green' : ''}` }>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <form onSubmit={handleCreatePlaylist} className="bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <div className="mb-4">
              ...
              <input type="text" className="w-full p-2 rounded bg-gray-800 text-white" required value={playlistName} onChange={e => setPlaylistName(e.target.value)} />
            </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-6">
        <ul className="space-y-2 text-spotify-text-secondary">
          {playlists.map((playlist) => (
            <li key={playlist.id} className="relative truncate hover:text-spotify-text-primary transition-colors flex items-center">
              <NavLink to={`/playlist/${playlist.id}`} className="text-sm flex-1 block truncate pr-8">{playlist.name}</NavLink>
            </li>
          ))}
