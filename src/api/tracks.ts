...
export const getTracks = async (): Promise<ApiResponse<Track[]>> => {
  try {
    ...
    return createResponse(USE_MOCK_DATA ? mockData.tracks : [], 
...
export const searchTracks = async (query: string): Promise<ApiResponse<Track[]>> => {
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/search?q=${encodeURIComponent(query)}&type=track`);
...
export const getTracksByAlbum = async (albumId: string): Promise<ApiResponse<Track[]>> => {
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/albums/${albumId}/tracks`);
...
export const getTopTracks = async (artistId: string): Promise<ApiResponse<Track[]>> => {
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/artists/${artistId}/top-tracks?market=US`);
    return createResponse(response.data.tracks);
  } catch (error) {
...
export const getRecommendations = async (seedTrackIds: string[]): Promise<ApiResponse<Track[]>> => {
  try {
    if (USE_MOCK_DATA) {
      // Simple recommendation algorithm: get tracks from the same artists
      const seedTracks = seedTrackIds.map(id => mockData.tracks.find(t => t.id === id)).filter(Boolean) as Track[];
      const artistIds = [...new Set(seedTracks.map(track => track.artistId))];
      
      const recommendations = mockData.tracks.filter(track => 
        artistIds.includes(track.artistId) && !seedTrackIds.includes(track.id)
      ).slice(0, 10);
    ...
    const response = await axios.get(`${API_BASE_URL}/recommendations?seed_tracks=${seedTracks}&limit=10`);
    return createResponse(response.data.tracks);
  } catch (error) {
