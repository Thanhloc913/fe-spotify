...
export const getAlbumById = async (
  ...
  try {
    ...
    if (
      tracksResponse.data.success &&
      tracksResponse.data.data &&
      tracksResponse.data.data.result
    ) {
      ...
      tracks = tracksData.map((track: any) => {
        return {
          ...
          popularity: 100,
          coverUrl:
            track.coverUrl || track.backgroundUrl || album.coverUrl || "",
...
export const searchAlbums = async (
  ...
  try {
    ...
    const response = await axios.get(
      `${API_BASE_URL}/search?q=${encodeURIComponent(query)}&type=album`
    );
...
export const getNewReleases = async (): Promise<ApiResponse<Album[]>> => {
  try {
    if (USE_MOCK_DATA) {
      ...
      const sortedAlbums = [...mockData.albums].sort(
        (a, b) =>
...
export const getAlbumsByArtist = async (
  ...
  try {
    if (USE_MOCK_DATA) {
      const albums = mockData.albums.filter(
