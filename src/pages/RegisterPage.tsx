...
import { FaSpotify, FaGoogle, FaFacebook, FaApple, FaEye, FaEyeSlash } from 'react-icons/fa';
...
const RegisterPage: React.FC = () => {
  ...
  const validateStep2 = () => {
    ...
    const hasNumberOrSpecial = /[0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+/.test(pw);
    const longEnough = pw.length >= 10;
    return (
      hasLetter && hasNumberOrSpecial && longEnough &&
      formData.fullName.trim() !== '' &&
      formData.day && formData.month && formData.year && formData.gender
    );
  };
  ...
  return (
    <div className="min-h-screen max-h-screen flex justify-center bg-black overflow-y-auto">
      <div className="w-full max-w-md bg-[#121212] rounded-lg shadow-lg p-8 flex flex-col items-center mt-8 mb-8">
        ...
        {step === 1 && (
          <>
            <h1 className="text-3xl font-bold text-white mb-6 text-center">Đăng ký để bắt đầu nghe</h1>
            <form className="w-full flex flex-col gap-4" onSubmit={e => { e.preventDefault(); handleNext(); }}>
              {error && <div className="text-red-500 text-center">{error}</div>}
              <div>
                <label className="block text-white font-bold mb-1">Địa chỉ email</label>
                <input
                  type="email"
                  name="email"
                  ...
                  required
                />
                <a className="text-spotify-green text-sm mt-1 inline-block cursor-pointer hover:underline">Dùng số điện thoại.</a>
              </div>
              <button
                type="submit"
            ...
            <div className="w-full flex flex-col gap-3 mb-2">
              ...
              <button className="flex items-center justify-center gap-2 w-full py-2 rounded-full border border-gray-700 text-white hover:bg-gray-800 transition">
                <FaFacebook className="text-lg text-blue-500" /> Đăng ký bằng Facebook
              </button>
              <button className="flex items-center justify-center gap-2 w-full py-2 rounded-full border border-gray-700 text-white hover:bg-gray-800 transition">
        ...
        {step === 2 && (
          <>
            <div className="w-full">
              ...
              <div className="text-2xl font-bold text-white mb-6">Tạo mật khẩu</div>
              <form className="w-full flex flex-col gap-4" onSubmit={e => { e.preventDefault(); handleNext(); }}>
                {error && <div className="text-red-500 text-center">{error}</div>}
                <div className="relative">
                  <label className="block text-white font-bold mb-1">Mật khẩu</label>
                  <input
                ...
                <div className="text-white text-sm mb-2">Mật khẩu của bạn phải có ít nhất</div>
                <ul className="mb-2">
                  <li className={/[a-zA-Z]/.test(formData.password) ? 'text-green-400' : 'text-gray-400'}>✔ 1 chữ cái</li>
                  <li className={/[0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+/.test(formData.password) ? 'text-green-400' : 'text-gray-400'}>✔ 1 chữ số hoặc ký tự đặc biệt (ví dụ: # ? ! &amp;)</li>
                  <li className={formData.password.length >= 10 ? 'text-green-400' : 'text-gray-400'}>✔ 10 ký tự</li>
                </ul>
                <div>
                ...
                <div>
                  <label className="block text-white font-bold mb-1">Ngày sinh</label>
                ...
                <div>
                  <label className="block text-white font-bold mb-1">Giới tính</label>
                  <div className="flex flex-wrap gap-4 mt-2">
                    {GENDERS.map(g => (
                      <label key={g.value} className="flex items-center gap-2 text-white">
                        <input
                          type="radio"
                          name="gender"
                          value={g.value}
                ...
                <div className="flex gap-2 mt-4">
                  <button type="button" onClick={handleBack} className="flex-1 py-3 rounded-full font-bold text-lg bg-gray-800 text-white hover:bg-gray-700 transition">Quay lại</button>
                  <button type="submit" className="flex-1 bg-spotify-green text-black py-3 rounded-full font-bold text-lg hover:scale-105 transition">Tiếp theo</button>
                </div>
              </form>
            </div>
          </>
        ...
        {step === 3 && (
          <>
            <div className="w-full">
              ...
              <div className="text-2xl font-bold text-white mb-6">Điều khoản & Điều kiện</div>
              <form className="w-full flex flex-col gap-4" onSubmit={handleSubmit}>
                ...
                <div className="text-gray-400 text-sm mt-2">
                  ...
                  <a href="#" className="text-spotify-green underline">Điều khoản và điều kiện sử dụng</a> của Spotify.<br />
                  Để tìm hiểu thêm về cách thức Spotify thu thập, sử dụng, chia sẻ và bảo vệ dữ liệu cá nhân của bạn, vui lòng xem{' '}
                  <a href="#" className="text-spotify-green underline">Chính sách quyền riêng tư của Spotify</a>.
                </div>
                <div className="flex gap-2 mt-4">
                  <button type="button" onClick={handleBack} className="flex-1 py-3 rounded-full font-bold text-lg bg-gray-800 text-white hover:bg-gray-700 transition">Quay lại</button>
                  <button type="submit" className="flex-1 bg-spotify-green text-black py-3 rounded-full font-bold text-lg hover:scale-105 transition">Đăng ký</button>
                </div>
              </form>
            </div>
          </>
